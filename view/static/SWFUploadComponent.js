...
// REQUIRES THIS EXTERNAL FILE: 	
// name: 'SWFUpload'
// URL: '/jslib/SWFUpload/swfupload.js' 
// need to figure out how to dynamically load this with Extjs4
...
Ext.define('Ext.ux.SWFUpload', {
    ...
    config: {
        ...
        fieldName: 'mediaFile',
        swfUrl: 'js/static/swfupload.swf',
        swfUploadCfg: {},
        relayCookies: false,
        baseParams: [],
        progressBar: false,
    ...
    initComponent: function () {
        if (!this.relayCookies) this.relayCookies = ['s'];
        ...
        return Ext.apply({
            ...
            file_dialog_complete_handler: Ext.bind(function (numFilesSelected, numFilesQueued, queueTotal) {
                ...
                if (false !== this.fireEvent('filesSelected', numFilesSelected, numFilesQueued, queueTotal) && this.progressBar) {
                    if (numFilesQueued) this.progressBar.updateProgress(0, 'Preparing to upload ' + queueTotal + (queueTotal > 1 ? ' files...' : ' file...'), false);
                    else this.progressBar.updateProgress(0, 'Ready to upload media', false);
                }
